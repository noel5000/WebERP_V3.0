...
namespace WebERPV3.Repository
{
    public class SupplierReturnRepository : Repository<SupplierReturn>, ISupplierReturnRepository
    {
        ...
        public SupplierReturnRepository(MainContext context, IDataRepositoryFactory repositoryFactory) : base(context)
        {
        ...
        public async Task<Result<object>> AddInventoryList(List<SupplierReturn> entries, string reference, string details)
        {
            ...
            e.UnitId,
              e.Product.ProductUnits.ToList()
                    ...
                    if (entries.Count > 0)
                    {
                        await _Context.SuppliersReturns.AddRangeAsync(entries);
        ...
        public async Task<Result<object>> RemoveEntries(string sequence)
        {
            ...
            using (var tran = await _Context.Database.BeginTransactionAsync())
            {
                try
                {
                    var entries = await _Context.SuppliersReturns.AsNoTracking().Where(x => x.IsDeleted == false && x.Sequence == sequence).ToListAsync();
                    entries.ForEach( async e =>
                    {
                        e.IsDeleted = true;
...
productUnits
        ...
        public override async Task< Result<SupplierReturn>> Add(SupplierReturn entity)
        {
            return await Task.Factory.StartNew<Result<SupplierReturn>>(() => {
                throw new NotImplementedException();
            });
        }

        public override async Task<Result<SupplierReturn>> Update(SupplierReturn entity)
        {
            return await Task.Factory.StartNew<Result<SupplierReturn>>(() => {
                throw new NotImplementedException();
            });
